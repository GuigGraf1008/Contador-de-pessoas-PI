...
# Obter largura do vídeo
width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))

# Centralizar a linha de detecção horizontalmente
posW = width // 2

offset = 50  # Deslocamento permitido

xy1 = (posW, 500)  # Ponto inicial da linha
xy2 = (posW, 100)  # Ponto final da linha
...
while True:
    ...
    cv2.line(frame, (posW - offset, xy1[1]), (posW - offset, xy2[1]), (255, 255, 0), 2)
    ...
    for cnt in contours:
        ...
        if int(area) > 3000:
            ...
            if len(detects) <= i:
                detects.append([])
            if centro[0] > posW - offset and centro[0] < posW + offset:
    ...
    if len(contours) == 0:
        ...
        for detect in detects:
            for (c, l) in enumerate(detect):
                if detect[c - 1][0] < posW and l[0] > posW:
                    detect.clear()
                    left += 1
                    total += 1
    ...
    cv2.putText(frame, "LEFT: " + str(left), (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    cv2.putText(frame, "RIGHT: " + str(right), (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
